...
export function Leaderboard({ currentWallet }: LeaderboardProps) {
  ...
  return (
    <div className="bg-white border-2 border-black shadow-[3px_3px_0px_0px_#000] sm:shadow-[4px_4px_0px_0px_#000] rounded-lg xs:rounded-xl sm:rounded-2xl overflow-hidden h-full flex flex-col">
      {/* Header */}
      <div className="bg-black px-2 xs:px-3 sm:px-4 py-1.5 xs:py-2 sm:py-2 md:py-3 flex items-center gap-1.5 xs:gap-2 flex-shrink-0">
        <h2 className="text-sm xs:text-base sm:text-base md:text-lg lg:text-xl font-display text-white m-0 tracking-wide">Top Catchers</h2>
      </div>

      {/* List */}
      <div className="p-1.5 xs:p-2 sm:p-2 md:p-3 space-y-0.5 xs:space-y-1 overflow-y-auto flex-1 min-h-0">
          ...
          return (
            <div
              ...
              className={`
                ...
                ${isCurrentUser ? "bg-gray-800 text-white" : "hover:bg-black/[0.03] active:bg-black/[0.05]"}
            ...
            >
              ...
              <div className="flex items-center gap-0.5 xs:gap-1 sm:gap-1.5 shrink-0">
                {isTop3 && <AlgorandMark size={9} className="xs:w-[10px] xs:h-[10px] sm:w-[11px] sm:h-[11px]" color={isCurrentUser ? "#fff" : idx === 0 ? "#000" : "#666"} />}
                <span
